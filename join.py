...
def prepare_data(path1, path2, join_col, join_type):
    ...
    if join_type in ("inner", "left"):
        ...
        final_headers = (
            header1
            + ","
            + header2.replace(join_col + ",", "").replace("," + join_col, "")
        )
...
def inner_join(first_path, second_path, first_join_col_idx, second_join_col_idx):
    ...
    for first_row in first_gen:
        ...
        for second_row in second_gen:
            ...
            if (
                first_row_split[first_join_col_idx]
                == second_row_split[second_join_col_idx]
            ):
...
def perform_join(first_filepath, second_filepath, join_col, join):
    # data preparation
    first_join_col_idx, second_join_col_idx, final_headers = prepare_data(
        first_filepath, second_filepath, join_col, join
    )
    ...
    if join == "inner":
        inner_join(
            first_filepath, second_filepath, first_join_col_idx, second_join_col_idx
        )
    elif join == "left":
        left_join(
            first_filepath, second_filepath, first_join_col_idx, second_join_col_idx
    ...
    # join columns indexes swapped with each other
        ...
        left_join(
            second_filepath, first_filepath, second_join_col_idx, first_join_col_idx
        )
